...
**Depends on**: Phase 0
**Requirements**: REPR-01, REPR-02, REPR-03, REPR-04, REPR-05, REPR-06, REPR-07, REPR-08, REPR-09, REPR-09a, REPR-10, REPR-11
**Success Criteria** (what must be TRUE):
...
**Plans**: 6 plans, serial (waves 1-6; each builds on the definer the previous one landed, and every plan after 01-01 appends to `model/Scales.gms`)

Plans:
- [ ] 01-01-PLAN.md — `model/Scales.gms` as the sole definer + the 53-bit rule stated once + `power(2,k)`-only constants + REPR-09a's reject-never-truncate carriage policy *(wave 1, REPR-09/REPR-09a/REPR-01)*
- [ ] 01-02-PLAN.md — `SATURATION_SENTINEL` re-derived at build time and checked against `model/spec-pins/saturation.pin`; `make check-saturation`; both overflow controls *(wave 2, REPR-06)*
- [ ] 01-03-PLAN.md — four-concept tick bounds + two-level spacing domain with data-driven values + Δᵢ=200 reachability *(wave 3, REPR-03/REPR-05)*
- [ ] 01-04-PLAN.md — η canonicalization with two named bridges, `inventory` reconciliation and co-compilation, the tick-sensitivity assertion *(wave 4, REPR-02/REPR-04)*
- [ ] 01-05-PLAN.md — provenance: mutation proof or recorded waiver for every unloaded symbol; **REPR-07 delivered as the units bridge — units ANSWERED, normalizer anchor recorded as a Phase 4 dependency**; `assertAdd0Branch` *(wave 5, REPR-08/REPR-07/REPR-11)*
- [ ] 01-06-PLAN.md — REPR-10 — the 181-point comparison, with the **reference side sourced from the committed table** (correspondence: Plank at Δᵢ ↔ tick `iCfg·Δᵢ`; a Lean-macro reference is bit-identical to the Plank one and measured **vacuous even under a wrong λ**); the exact `tick → sqrtPriceX96` **table** generated outside GAMS, self-verified at three known constants, regenerated by `check-fixtures`, **shared with VPATH-13 in Phase 11 — one artifact**; both bridges retired, with a wrong-λ control that reddens *(wave 6, REPR-10)*. **Not** a `TickMathReplica.gms`: criterion (b) establishes that a GAMS-computed replica is common-mode at the same 53-bit mantissa it exists to check.
