...
| REPR-07 | Phase 1 — Representation kernel + spine | Pending | **UNITS ANSWERED; NORMALIZER UNANCHORED.** `plank/notes/UNITS_AND_SCALES.md` settles the units question: on-chain L̄ is raw u128-native liquidity, collateral lives in the prices, so `LbarQ128 = Q128` is a normalized fixture and `2^128` is correct — plan **01-05** delivers that as an executable bridge (normalizer named, `liquidity_for_collateral` named, out-of-u128 value REFUSED) and does **not** record both candidate readings. What remains open is the numeric **anchor**: `lbarScaleRaw` is this model's own choice with nothing tying it to `E2.liquidityBar`, because E2 is SPEC-ONLY. Anchor is a **Phase 4 dependency** (DATA-09's E2 linkage); `cfmm-gams#1` moves from BLOCKED to *units answered, anchor outstanding*, not to closed |
