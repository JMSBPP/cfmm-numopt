...
- [ ] 01-05-PLAN.md — provenance: mutation proof or recorded waiver for every unloaded symbol; **REPR-07 delivered as the units bridge (ANSWERED, not re-asked)**; `assertAdd0Branch` *(wave 5, REPR-08/REPR-07/REPR-11)*
- [ ] 01-06-PLAN.md — REPR-10 — the 181-point cross-representation comparison (correspondence is Plank at Δᵢ ↔ Lean at Δᵢ/2); the exact `tick → sqrtPriceX96` **table** generated outside GAMS, self-verified at three known constants, regenerated by `check-fixtures`, **shared with VPATH-13 in Phase 11 — one artifact**; both bridges retired, with a wrong-λ control that reddens *(wave 6, REPR-10)*. **Not** a `TickMathReplica.gms`: criterion (b) establishes that a GAMS-computed replica is common-mode at the same 53-bit mantissa it exists to check.
